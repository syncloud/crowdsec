...
def module_setup(request, device, app_dir, artifact_dir):
    def module_teardown():
        device.run_ssh('ls -la /var/snap/crowdsec/current/config > {0}/config.ls.log'.format(TMP_DIR), throw=False)
        device.run_ssh('cp /var/snap/crowdsec/current/config/crowdsec.yaml {0}/crowdsec.yaml.log'.format(TMP_DIR), throw=False)
        ...
        device.run_ssh('ls -la /snap/crowdsec > {0}/snap.crowdsec.ls.log'.format(TMP_DIR), throw=False)
        device.run_ssh('ls -la /var/snap > {0}/var.snap.ls.log'.format(TMP_DIR), throw=False)
        device.run_ssh('ls -la /var/snap/crowdsec > {0}/var.snap.crowdsec.ls.log'.format(TMP_DIR), throw=False)
        device.run_ssh('ls -la /var/snap/crowdsec/current/ > {0}/var.snap.crowdsec.current.ls.log'.format(TMP_DIR),
                       throw=False)
        device.run_ssh('ls -la /var/snap/crowdsec/common > {0}/var.snap.crowdsec.common.ls.log'.format(TMP_DIR),
                       throw=False)
        device.run_ssh('ls -la /data > {0}/data.ls.log'.format(TMP_DIR), throw=False)
        device.run_ssh('ls -la /data/crowdsec > {0}/data.crowdsec.ls.log'.format(TMP_DIR), throw=False)

        app_log_dir = join(artifact_dir, 'log')
        os.mkdir(app_log_dir)
        device.scp_from_device('/var/snap/crowdsec/common/log/*.log', app_log_dir)
...
def __log_data_dir(device):
    ...
    device.run_ssh('ls -la /data/')
    device.run_ssh('ls -la /data/crowdsec')


def test_storage_change_event(device):
    device.run_ssh('snap run crowdsec.storage-change > {0}/storage-change.log'.format(TMP_DIR))
...
def test_hub_update(device):
    device.run_ssh('snap run crowdsec.cscli hub update')


def test_hub_install(device):
    device.run_ssh('snap run crowdsec.cscli collections install crowdsecurity/dovecot')


def test_hub_list(device):
    device.run_ssh('snap run crowdsec.cscli hub list | grep dovecot')
