...
```
curl -s "http://ci.syncloud.org:8080/api/repos/syncloud/crowdsec/builds/{N}" | python3 -c "
import json,sys
...
```
curl -s "http://ci.syncloud.org:8080/api/repos/syncloud/crowdsec/builds/{N}/logs/{stage}/{step}" | python3 -c "
...
http://ci.syncloud.org:8080/syncloud/crowdsec

CI is Drone CI (JS SPA). Check builds via API:
```
curl -s "http://ci.syncloud.org:8080/api/repos/syncloud/crowdsec/builds?limit=5"
```
...
```
curl -s "http://ci.syncloud.org:8081/files/crowdsec/{build}-{arch}/"
```

Each distro dir contains `app/`, `platform/`, and for upgrade/UI tests also `desktop/`, `refresh.journalctl.log`, `video.mkv`:
```
curl -s "http://ci.syncloud.org:8081/files/crowdsec/{build}-{arch}/{distro}/"
curl -s "http://ci.syncloud.org:8081/files/crowdsec/{build}-{arch}/{distro}/app/"
curl -s "http://ci.syncloud.org:8081/files/crowdsec/{build}-{arch}/{distro}/desktop/"
```
...
```
curl -O "http://ci.syncloud.org:8081/files/crowdsec/282-amd64/buster/app/journalctl.log"
curl -O "http://ci.syncloud.org:8081/files/crowdsec/282-amd64/bookworm/desktop/journalctl.log"
```
...
drone exec --pipeline amd64 --trusted \
  ...
  --include "build java" \
  --include "build nginx" \
  --include "build crowdsec" \
  --include "build metabase" \
  --include cli \
  --include package \
